...
interface ChecklistProps {
  items: ChecklistItem[];
  onUpdate: (id: number, completat: boolean, url?: string) => Promise<void>;
}
...
export default function Checklist({ items, onUpdate }: ChecklistProps) {
  ...
  const handleToggle = async (item: ChecklistItem) => {
    setLoading(item.id_checklist);
    try {
      await onUpdate(item.id_checklist, !item.completat, item.url_evidencia);
    } finally {
      setLoading(null);
    }
  };
