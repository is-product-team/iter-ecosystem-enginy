...
export default function StudentDashboardPage() {
    ...
    return (
        <DashboardLayout
            ...
            subtitle="View your workshops and download your certificates."
        >
                ...
                <section className="bg-white border-2 border-gray-100 p-8 shadow-sm relative overflow-hidden mb-8">
                     ...
                     {certificates.length === 0 ? (
                        <div className="p-10 text-center bg-gray-50 border border-gray-100">
                             <p className="text-[11px] font-black uppercase text-gray-400 tracking-[0.2em] mb-2">No certificates found.</p>
                             <p className="text-xs text-gray-500">You haven't obtained any certificates yet. Complete a workshop to earn one.</p>
                        </div>
                     ) : (
                             ...
                             {certificates.map(cert => (
                                 <div key={`${cert.studentId}-${cert.assignmentId}`} className="border-2 border-gray-100 p-6 flex flex-col justify-between hover:border-[#00426B] transition-colors">
                                     <div>
                                         ...
                                         <p className="text-xs text-gray-500 mb-6">Duration: {cert.assignment?.workshop?.durationHours}h</p>
                                     </div>
                                     <div>
                                         <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-3">Issued: {new Date(cert.issuedAt).toLocaleDateString()}</p>
                                         <button 
                                            onClick={() => toast.info('Certificate PDF generation is not implemented in this demo.')}
