...
export default function AdminIssueDetailPage() {
  ...
  return (
    ...
    >
      <div className="flex flex-col lg:flex-row h-[calc(100vh-180px)] gap-8 animate-in fade-in duration-700 font-sans">
        
        {/* Main Chat Area - Style Apple */}
        <div className="flex-1 flex flex-col bg-background-surface border border-border-subtle shadow-sm overflow-hidden rounded-2xl">
           {/* Chat Header Info Mobile */}
           <div className="p-5 border-b border-border-subtle bg-background-subtle/50 flex justify-between items-center sm:px-8 lg:hidden">
              ...
              <span className="text-[11px] font-medium text-consorci-darkBlue uppercase tracking-tight">{issue.center?.name}</span>
           </div>

           <div className="flex-1 overflow-y-auto p-6 space-y-10 sm:p-10 custom-scrollbar">
                  ...
                  <div className={`max-w-[80%] lg:max-w-[70%] flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                    ...
                    <div className={`p-5 rounded-[22px] ${
                      isMe 
                        ...
                        : 'bg-background-subtle text-text-primary rounded-tl-md border border-border-subtle/50 shadow-sm'
                    }`}>
        ...
        {/* Sidebar Controls - Clean Apple Style */}
        <div className="w-full lg:w-80 space-y-6">
           {/* Center Card */}
           <div className="bg-background-surface border border-border-subtle p-8 rounded-2xl shadow-sm">
              <h4 className="text-[10px] font-medium text-text-muted uppercase tracking-[0.2em] mb-6">{t('table.center')}</h4>
              <div className="flex flex-col gap-1">
                 <span className="text-lg font-medium text-text-primary tracking-tight">{issue.center?.name}</span>
                 <span className="text-[13px] font-normal text-text-muted opacity-60">{issue.creator?.fullName}</span>
              </div>
           </div>

           {/* Controls Card */}
           <div className="bg-background-surface border border-border-subtle p-8 space-y-10 rounded-2xl shadow-sm">
              <div className="space-y-4">
                 <label className="text-[10px] font-medium text-text-muted uppercase tracking-[0.2em] block">{t('table.status')}</label>
                 <select 
                   value={issue.status}
                   onChange={(e) => handleUpdateStatus(e.target.value)}
                   className="w-full p-4 bg-background-subtle border border-border-subtle text-[11px] font-medium uppercase tracking-[0.1em] outline-none focus:border-consorci-darkBlue cursor-pointer transition-all hover:bg-background-page rounded-xl appearance-none"
                 >
              ...
              <div className="space-y-4">
                 <label className="text-[10px] font-medium text-text-muted uppercase tracking-[0.2em] block">{t('table.priority')}</label>
                 <div className="grid grid-cols-2 gap-3">
                    {Object.values(ISSUE_PRIORITIES).map(p => (
                       <Button
                         ...
                         size="sm"
                         className="!py-3 !text-[9px] font-medium uppercase tracking-widest transition-all rounded-xl shadow-none"
                       >
                          {p}
                       </Button>
                    ))}
              ...
              <div className="pt-6 border-t border-border-subtle">
                 <div className="text-[10px] font-normal text-text-muted uppercase tracking-wider flex items-center gap-2 opacity-50">
                    <Clock size={12} />
